...
이것들은 입안에서 엷은 세균막(Biofilm)을 만드는데,
놔두면 점점 뭉쳐 조직을 이루고,
조직을 이룬 세균은 이에 해로운 작용을 해 썩게 만든다.

![Dental Biofilm](https://blog.dentalplans.com/wp-content/uploads/2016/10/2015-10-Biofilm-teeth.jpg "프라크(Plaque)로 많이 알려진 세균막이 모든 이 문제의 원인이다.")
...
333은 하루 3번, 먹고 3분 안에, 적어도 3분 이상 이를 닦으라는 것을 의미한다.
...
그러니 설사 입안 세균을 모두 없앴다고 해도, 금세 다시 세균이 들어오게 될 것이다.

대신, 세균을 관리할 수는 있다.
해로운 작용을 하는 세균 조직이 생기지 않게 이에 달라붙은 세균들을 흩트려 주는 거다.
그와 함께 이에 붙어있던 세균막을 벗겨내 치석이 생기지 않도록 해준다.
세균막은 금세 다시 생기겠지만, 쌓이지만 않는다면 치석이 되지는 않기 때문이다.
...
설거지할 때 그릇을 하나씩 집어 닦듯이 칫솔질도 이 하나씩 닦아야 한다.
...
익숙지 않다면 10분 혹은 15분이 걸릴 수도 있는데, 시간에 신경 쓰지 말고 제대로 닦는 것에만 신경 쓴다.
...
이를 닦는 시간과 횟수는 하루 2번, 아침과 저녁에 닦는다.
칫솔질의 목적이 세균막을 벗겨내고 세균들을 흩트려 주는 것이므로,
12시간에 한 번 정도 해준다는 느낌으로 하면 된다.
정 여의치 않다면 자기 전에 한 번만이라도 신경 써서 잘 닦는 게 좋다.

이를 닦는 시기는 음식을 먹고 3분 이내가 가장 좋으며,
...
음식을 먹은 후 pH는 급격히 떨어져 약 3분 후 pH 5.5 이하로 떨어지는데,
이때 칫솔질을 하면 치아 마모가 심했다.
앞서 소개한 333의 '3분 이내'도 여기서 나온 것이다.
그러나, 밥을 먹고 바로 이를 닦을 수 없다면,
물도 마시고 하면서 pH가 안정화된 2~30분 후에 닦는 것이 차라리 나을 수 있다.

### 올바른 칫솔질을 위한 도구 선택법

그렇다면, 이런 칫솔질에 유용한 칫솔과 치약은 뭘까.
...
그래서 이와 닿는 면적은 자체는 넓지 않아 별 효과를 보긴 어렵다.
...
만약 끝이 과하게 벌어졌다면, 너무 힘줘서 닦고 있는 건 아닌지 의심해봐야 한다.

#### 좋은 치약 고르는 법

치약에서 주의 깊게 봐야 할 것은 계면활성제다.
계면활성제는 수용성 물질과 지용성 물질을 섞이게 만들어
원래는 물로 닦을 수 없던 것도 물로 씻어낼 수 있게 해주는 물질로,
대게 로릴 황산나트륨(Sodium Lauryl Sulfate, SLS),
로레트 황산나트륨(Sodium Laureth Sulfate, SLES),
도데실 황산나트륨(Sodium Dodecyl Sulfate, SDS) 등을 사용한다.
...
간단하게는 비누라고 생각하면 쉽다.
칫솔질할 때 거품이 난다면 100% 계면활성제가 들어있다고 보면 된다.
이를 닦고 나서 음식 맛이 이상하다면 역시 계면활성제가 들은 것으로 볼 수 있다.
이는 계면활성제가 미각 세포를 잠시 마비시키기 때문에 생기는 현상이다.

잘 닦이게 해주는데 뭐가 문제냐면,
이게 피부 자극성 물질이라 염증을 일으키기 때문이다.
이는 천연 계면활성제라고 하는 것도 크게 다르지 않으니,
계면활성제는 포함되어있지 않은 것을 사용하는 게 좋다.

또, 연마재는 적을수록 좋다.
마모도가 30 이하인 걸 권하며, 70 이상이라면 안 쓰는 게 좋다.
이를 하얗게 만들기는 하지만, 그만큼 이가 손상되기 때문이다.
치약의 미백 원리가 착색된 이 표면을 깎아내 속에 있던 하얀 부분을 드러내는 것이라서다.
매일 이를 깎아내는 게 이 건강에 좋을 리 없다.
...
그 제품은 연마재가 많이 들어있는 것이라고 보면 된다.

반면에 최근 논란이 있는 불소(Fluorine)는 오히려 1000ppm 정도 되는 걸 고르는 게 좋다.
치약의 사용 이유 중 하나가 마모된 이를 보완해주고 코팅해주는 보충제이기 때문이다.
불소는 이를 코팅해 세균에 강하게 해준다.
불소가 없다면 오히려 치약으로서 어떤 효능을 내려는 것인지 의심해봐야 한다.

불소 섭취에 대한 우려가 퍼져있고
유해성에 대해서 확실한 연구나 결과가 있는 것은 아니나,
이를 닦고 나서 치약 대부분을 뱉어낸다는 걸 생각하면
치약으로 인해 부작용을 일으킬 정도로 섭취할 일은 생기지 않을 것이다.
...
최소한 필요 내용은 최대한 전달하기 위해 잡은 최소 시간이라는 생각이 들었다.
